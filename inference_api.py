...
class Inferener(object):
    ...
    def init_category_embedding(self, cat_emb_path):
        ...
        for i in range(cat_emb_trained.size(0)):
            self.cat_embedding_dict[str(i)] = cat_emb_trained[i]
    ...
    def cls_process_imgs(self, imgs):
        ...
        for im in imgs:
            ...
            img_transform = get_transform(
                self.option, transform_params, grayscale=False
            )
            im = img_transform(im)
...
def test_with_specified_chars(
    ...
    true_inferencer.save_img(
        out_img, "{}_style_{}_infered.png".format(prefix, style_idx)
    )
